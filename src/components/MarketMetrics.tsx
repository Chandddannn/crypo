...
export function MarketMetrics({ asset }: MarketMetricsProps) {
  ...
  const metrics = [
    ...
    },
    {
      ...
      value: `${formatNumber(data.circulating_supply, 0)}`,
    ...
    <div className="glass-panel overflow-hidden transition-colors">
      <div className="px-6 py-5 border-b border-slate-100/50 transition-colors">
        <div className="flex items-center gap-3">
          ...
          <h3 className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-800 transition-colors">Market Analytics</h3>
        </div>
      </div>
          ...
          {metrics.map((metric, i) => (
            <div key={i} className="group flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-3">
                <span className="text-lg font-black text-slate-900 tracking-tight tabular-nums transition-colors">
                  {metric.value}
                </span>
                {metric.percentage !== null && metric.percentage !== undefined && (
                  <div className="space-y-2">
                    <div className="h-1.5 w-full overflow-hidden rounded-full bg-slate-100 border border-slate-50 transition-colors">
                      <div 
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-black text-slate-400 uppercase tracking-tighter transition-colors">
                        Supply Progress
                      </span>
                      <span className="text-[9px] font-black text-indigo-600 tabular-nums transition-colors">
